---
title: "india_failure_combined_mDx_removed"
author: "avanvalken"
date: "6/4/2021"
...
## Remove those who missed treatments
```{r}
missed_subjid <- zf %>% mutate(new_miss = rowSums(zf[,c("FUA_DOSES30", "FUA_DOSES7", "totmiss")]))

missed <- missed_subjid[which(missed_subjid$new_miss > 0), "subjid"]
missed_baseline <- gsub("A2", "A0", missed)

missed <- c(missed, missed_baseline)

indata <- indata[,!(colData(indata)$subjid %in% missed)]
```
